...
Antes de que nada salga del equipo, pasa por **Alpha** (revisión interna: developer → segundo developer → lead) y luego **Beta** (revisión del cliente, IHG). El **Visual QA** tiene su propio checklist: headers, footer, T&Cs, links, CTAs, módulos, accesibilidad (alt text, ARIA labels). Todo el feedback se centraliza en el **Feedback Tracker (Smartsheet)** — nunca por mensaje suelto. Para la metodología completa de test cases (subject line, contenido, links, rendering cross-cliente, otros idiomas), ver [[LFC_20260623_GALE QA Test Case Methodology & Feedback Tracking]] y la nota general de [[QA Process]].
