...
| Término                                          | Qué es                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Briefing document**                            | Documento inicial que IHG entrega a GALE con los requisitos de la campaña — KPIs, canal, módulos históricos, librería de imágenes. Punto de partida de todo el ciclo.                                                                                                                    |
| **Comms plan**                                   | Spreadsheet que mapea la **secuencia completa de touch-points** de una campaña multi-touch (ej. anuncio → recordatorio de registro → recordatorio de booking → completion) y a qué stage pertenece cada uno. Lo arma el **campaign manager** del lado IHG (ej. Matt) y es una de las tres fuentes que nutren al CRF, junto con el List Pull y el Figma aprobado. Se consulta junto con el List Pull antes de construir un asset, para saber a qué stage pertenece y qué DE lo alimenta. Distinto del **Master Deck** (documento estratégico más amplio, uno por fase de Life Cycle: Educate/Engage/Nurture/Retain).                                                                                                                                                                                  |
...
| **List Pull / List Pool**                        | Documento que define qué campo de la Data Extension trae qué valor — la base para escribir tanto el AMPscript como los test cases. Lo **redacta/organiza el developer de GALE** (a solicitud del campaign manager, que mantiene la propiedad del requerimiento), y se **revisa y valida junto con el equipo de Datos**, que construye la Data Extension real siguiendo exactamente lo que dice el documento. Se prepara en paralelo a la revisión del Figma.                                                             |
...
| **Test Case Document**                           | El documento central del proceso de QA de GALE — segregado por email y por push, con hojas por cada email de la campaña e hipervínculos entre ellas.                                                                                                                                     |
...
| Rol / Equipo | Función |
|---|---|
| **GALE** | Agencia/equipo que ejecuta el desarrollo — oficinas en Guadalajara (delivery/PM) y Bangalore (~90% del build y del testing). |
...
Tabla completa de locale → region/language code → path web, con su fuente oficial (SharePoint IHG), ahora vive en su propia nota: **[[Region Language Codes]]**.
...
| **Beta** | Ronda de revisión con el cliente (IHG / campaign manager). |
...
`Club` → `Silver` → `Gold` → `Platinum` → `Diamond` — niveles de estatus del programa de lealtad IHG One Rewards, usados constantemente como base de personalización dinámica y de elegibilidad de audiencia en las campañas LFC.
