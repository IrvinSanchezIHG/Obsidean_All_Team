...
- **Alias y link helper**: cada anchor tag lleva, además de la URL final, un **alias name** que sigue la convención del Link/URL Matrix del CRF (ej. `mod1_headlineBonus`) y, cuando aplica, un título legible al hover. Las URLs se arman con un **link helper** compartido que resuelve automáticamente la variante correcta según el código de región/idioma — la misma referencia de link puede resolver a la página global en inglés o a la página en japonés sin mantener una URL distinta por idioma.
